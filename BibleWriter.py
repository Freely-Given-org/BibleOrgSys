        ...
        def __formatMarkdownVerseText( BBB, C, V, givenText, extras ):
            ...
            def handleExtras( text, extras ):
                ...
                def processXRef( MDxref ):
                    ...
                    """
                    markerList = BibleOrgSysGlobals.USFMMarkers.getMarkerListFromText( MDxref, includeInitialText=True )
...
#                    verse = verse[:ix1] + osisFootnote + verse[ix2b:]
    ...
    def __formatHTMLVerseText( BBB, C, V, givenText, extras, ourGlobals ):
        ...
        def handleExtras( text, extras, ourGlobals ):
            ...
            def processXRef( HTML5xref, ourGlobals ):
                ...
                <p id="XRef0" class="XRef"><a title="Go back up to 2:2 in the text" href="#C2V2"><span class="ChapterVerse">2:2</span></a> <span class="VernacularCrossReference">Lib 19:9&#x2011;10</span>; <span class="VernacularCrossReference">Diy 24:19</span></p>
                """
                markerList = BibleOrgSysGlobals.USFMMarkers.getMarkerListFromText( HTML5xref, includeInitialText=True )
    ...
    def toHTML5( self, outputFolder=None, controlDict=None, validationSchema=None, humanReadable=True ):
        ...
        if ignoredMarkers:
            logging.info( "toHTML5: Ignored markers were {}".format( ignoredMarkers ) )
            if BibleOrgSysGlobals.verbosityLevel > 2:
            ...
            def handleSection( sectionHTML, outputFile ):
                ...
                if BibleOrgSysGlobals.debugFlag: # Write this HTML section uncompressed in a separate folder (for debugging)
                    ...
                    if checkHTML != sectionHTML:
                        ...
                        halt
                #if BibleOrgSysGlobals.debugFlag: compressedHTML = sectionHTML # Leave it uncompressed so we can easily look at it
                if BibleOrgSysGlobals.debugFlag: compressedHTML += '\n'
    ...
    def toUSFXXML( self, outputFolder=None, controlDict=None, validationSchema=None ):
        ...
        def writeUSFXBook( xw, BBB, bkData ):
            ...
            USFXAbbrev = BibleOrgSysGlobals.BibleBooksCodes.getUSFMAbbreviation( BBB ).upper()
            if not USFXAbbrev:
    ...
    def toOSISXML( self, outputFolder=None, controlDict=None, validationSchema=None ):
        ...
        if booksNamesSystemName and booksNamesSystemName!='None' and booksNamesSystemName!='Unknown': # default (if we know the book names system)
            ...
            getBookAbbreviationFunction = BibleOrgSysGlobals.BibleBooksCodes.getOSISAbbreviation
        ...
        #with open( SwLocFilepath, 'wt', encoding='utf-8' ) as SwLocFile:
            ...
            #for BBB in BOS.getBookList():
                #if BBB in self.books:
                    #SwLocFile.write( '{}={}\n'.format(BibleOrgSysGlobals.BibleBooksCodes.getEnglishName_NR(BBB), getBookNameFunction(BBB) ) ) # Write the first English book name and the language book name
            #SwLocFile.write( '\n[Book Abbrevs]\n' )
            #for BBB in BOS.getBookList():
                #if BBB in self.books:
        ...
        toOSISGlobals = { "verseRef":'', "XRefNum":0, "FootnoteNum":0, "lastRef":'', "OneChapterOSISBookCodes":BibleOrgSysGlobals.BibleBooksCodes.getOSISSingleChapterBooksList() } # These are our global variables
            ...
            for verseDataEntry in bkData._processedLines: # Process internal Bible data lines
                ...
                if marker in OFTEN_IGNORED_USFM_HEADER_MARKERS or marker in ('ie',):
                    ...
                    if not haveOpenIntro:
                        ...
                        haveOpenIntro = True
                    if text: writerObject.writeLineOpenClose( 'title', checkText(text),('level',marker[-1]) ) # Introduction heading
                    ...
                    if not haveOpenIntro: # Shouldn't happen but we'll try our best
                        logging.error( _("toOSIS: {} Have an {} not in an introduction section").format( toOSISGlobals['verseRef'], marker ) )
                        writerObject.writeLineOpen( 'div', [('type',"introduction"),('canonical',"false")] )
                        haveOpenIntro = True
                    if not haveOpenOutline: # Shouldn't happen but we'll try our best
                        logging.warning( _("toOSIS: {} Have an {} not in an outline section").format( toOSISGlobals['verseRef'], marker ) )
                        closeAnyOpenParagraph()
                        writerObject.writeLineOpen( 'div', ('type',"outline") )
                        writerObject.writeLineOpen( 'list' )
                        haveOpenOutline = True
    ...
    def toZefaniaXML( self, outputFolder=None, controlDict=None, validationSchema=None ):
        ...
        toZefGlobals = { 'verseRef':'', 'XRefNum':0, 'FootnoteNum':0, 'lastRef':'', 'OneChapterOSISBookCodes':BibleOrgSysGlobals.BibleBooksCodes.getOSISSingleChapterBooksList() } # These are our global variables
        ...
        def writeZefBook( writerObject, BBB, bkData ):
            ...
            def checkText( textToCheck, checkLeftovers=True ):
                ...
                #if '\\bk ' in adjText: adjText = checkTextHelper('bk',adjText).replace('\\bk ','<reference type="x-bookName">').replace('\\bk*','</reference>')
                #if '\\ior ' in adjText: adjText = checkTextHelper('ior',adjText).replace('\\ior ','<reference>').replace('\\ior*','</reference>')
                if '\\add ' in adjText: adjText = checkTextHelper('add',adjText).replace('\\add ','<STYLE fs="italic">').replace('\\add*','</STYLE>') # ???
                if '\\nd ' in adjText: adjText = checkTextHelper('nd',adjText).replace('\\nd ','<STYLE fs="divineName">').replace('\\nd*','</STYLE>')
                #if '\\wj ' in adjText: adjText = checkTextHelper('wj',adjText).replace('\\wj ','<q who="Jesus" marker="">').replace('\\wj*','</q>')
                if '\\sig ' in adjText: adjText = checkTextHelper('sig',adjText).replace('\\sig ','<STYLE fs="italic">').replace('\\sig*','</STYLE>') # ???
                if '\\it ' in adjText: adjText = checkTextHelper('it',adjText).replace('\\it ','<STYLE fs="italic">').replace('\\it*','</STYLE>')
                if '\\bd ' in adjText: adjText = checkTextHelper('bd',adjText).replace('\\bd ','<STYLE fs="bold">').replace('\\bd*','</STYLE>')
                if '\\em ' in adjText: adjText = checkTextHelper('em',adjText).replace('\\em ','<STYLE fs="bold">').replace('\\em*','</STYLE>') # ???
                if '\\sc ' in adjText: adjText = checkTextHelper('sc',adjText).replace('\\sc ','<STYLE fs="small-caps">').replace('\\sc*','</STYLE>')
                if '\\fig ' in adjText: # Figure is not used in Sword modules so we'll remove it from the Zefania (for now at least)
                    ...
                    if ix2 == -1: logging.error( _("toZefania: Missing fig end marker for Zefania in {}: {!r} field is {!r}").format( toZefGlobals['verseRef'], marker, textToCheck ) )
                        ...
                        adjText = adjText[:ix1] + adjText[ix2+5:] # Remove the \\fig..\\fig* field
                        #print( "now {!r}".format( adjText ) )
                        logging.warning( _("toZefania: Figure reference removed for Zefania generation in {}: {!r} field").format( toZefGlobals['verseRef'], marker ) )
                if checkLeftovers and '\\' in adjText:
                    logging.error( _("toZefania: We still have some unprocessed backslashes for Zefania in {}: {!r} field is {!r}").format( toZefGlobals['verseRef'], marker, textToCheck ) )
...
# Temp disabled
#                                if not BRL.containsReference( BBB, currentChapterNumberString, verseNumberString ):
#                                    logging.error( _("toZefania: Cross-reference at {} {}:{} seems to contain the wrong self-reference anchor {!r}").format(BBB,currentChapterNumberString,verseNumberString, token[3:].rstrip()) )
                        elif lcToken.startswith('xt '): # xref text follows
                            xrefText = token[3:]
                            finalPunct = ''
                            while xrefText[-1] in ' ,;.': finalPunct = xrefText[-1] + finalPunct; xrefText = xrefText[:-1]
...
#                                    logging.error( _("toZefania: Footnote at {} {}:{} seems to contain the wrong self-reference anchor {!r}").format(BBB,currentChapterNumberString,verseNumberString, token[3:].rstrip()) )
                ...
                if extras:
                    ...
                    for extra in extras: # do any footnotes and cross-references
                        ...
                        lenV = len( verse )
                        if adjIndex > lenV: # This can happen if we have verse/space/notes at end (and the space was deleted after the note was separated off)
                            logging.warning( _("toZefania: Space before note at end of verse in {} has been lost").format( toZefGlobals['verseRef'] ) )
                            # No need to adjust adjIndex because the code below still works
                        elif adjIndex<0 or adjIndex>lenV: # The extras don't appear to fit correctly inside the verse
            ...
            for verseDataEntry in bkData._processedLines: # Process internal Bible data lines
                ...
                if marker in OFTEN_IGNORED_USFM_HEADER_MARKERS or marker in ('ie',): # Just ignore these lines
                    ...
                    if not text: logging.warning( "toZefania: Missing text for v" ); continue
...
# TODO: Some modules have <XMLBIBLE xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="zef2005.xsd" version="2.0.1.18" status='v' revision="1" type="x-bible" biblename="KJV+">
        ...
        if ignoredMarkers:
            logging.info( "toZefania: Ignored markers were {}".format( ignoredMarkers ) )
            if BibleOrgSysGlobals.verbosityLevel > 2:
...
# TODO: Some modules have <XMLBIBLE xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="zef2005.xsd" version="2.0.1.18" status='v' revision="1" type="x-bible" biblename="KJV+">
        ...
        if ignoredMarkers:
            logging.info( "toHaggai: Ignored markers were {}".format( ignoredMarkers ) )
            if BibleOrgSysGlobals.verbosityLevel > 2:
    ...
    def toSwordModule( self, outputFolder=None, controlDict=None, validationSchema=None ):
        ...
        #with open( SwLocFilepath, 'wt', encoding='utf-8' ) as SwLocFile:
            ...
            #for BBB in BOS.getBookList():
                #if BBB in self.books:
                    #SwLocFile.write( '{}={}\n'.format(BibleOrgSysGlobals.BibleBooksCodes.getEnglishName_NR(BBB), getBookNameFunction(BBB) ) ) # Write the first English book name and the vernacular book name
            #SwLocFile.write( '\n[Book Abbrevs]\n' )
            #for BBB in BOS.getBookList():
                #if BBB in self.books:
                    #SwLocFile.write( '{}={}\n'.format(BibleOrgSysGlobals.BibleBooksCodes.getEnglishName_NR(BBB).upper(), BibleOrgSysGlobals.BibleBooksCodes.getSwordAbbreviation(BBB) ) ) # Write the UPPER CASE language book name and the Sword abbreviation
...
#                    verse = verse[:ix1] + osisFootnote + verse[ix2b:]
            ...
            for verseDataEntry in bkData._processedLines: # Process internal Bible data lines
                ...
                if marker in OFTEN_IGNORED_USFM_HEADER_MARKERS or marker in ('ie',): # Just ignore these lines
                    ...
                    #    haveOpenIntro = False
                    if not haveOpenIntro: # Shouldn't happen but we'll try our best
                        logging.error( "toSwordModule: {} Have an {} not in an introduction section—opening an intro section".format( toSwordGlobals['verseRef'], marker ) )
                        writerObject.writeLineOpen( 'div', ('type',"introduction") )
                        haveOpenIntro = True
                    if not haveOpenOutline: # Shouldn't happen but we'll try our best
                        logging.warning( _("toSwordModule: {} Have an {} not in an outline section—opening an outline section".format( toSwordGlobals['verseRef'], marker ) ) )
                        closeAnyOpenParagraph()
                        writerObject.writeLineOpenSelfclose( 'div', [getSID(), ('type',"outline")] )
                        writerObject.writeLineOpen( 'list' )
                        haveOpenOutline = True
        ...
        if ignoredMarkers:
            logging.info( "toSwordModule: Ignored markers were {}".format( ignoredMarkers ) )
            if BibleOrgSysGlobals.verbosityLevel > 2:
    ...
    def toMyBible( self, outputFolder=None, controlDict=None ):
        """
        Using settings from the given control file,
    ...
    def toPhotoBible( self, outputFolder=None ):
        ...
        def renderChapterText( BBB, BBBnum, bookName, bookAbbrev, C, intC, maxChapters, numVerses, text, bookFolderName, fontsize=None ):
            ...
            if namingFormat == 'Short':
                ...
                chapterFolderName = chapterFoldernameTemplate.format( BibleOrgSysGlobals.BibleBooksCodes.getReferenceNumber( BBB ), intC, BBB )
        ...
        for BBB,bookObject in self.books.items(): # BBB is our three-character book code
            ...
            BBBnum = BibleOrgSysGlobals.BibleBooksCodes.getReferenceNumber( BBB )
            maxChapters = BibleOrgSysGlobals.BibleBooksCodes.getMaxChapters( BBB )
    ...
    def toODF( self, outputFolder=None ):
        ...
        def insertFormattedODFText( BBB, C, V, givenText, extras, documentText, textCursor, defaultCharacterStyleName ):
            ...
            def processCrossReference( rawXRef, documentText, textCursor ):
                ...
                \\x - \\xo 2:2: \\xt Lib 19:9-10; Diy 24:19.\\xt*\\x* (Backslashes are shown doubled here)
                """
                markerList = BibleOrgSysGlobals.USFMMarkers.getMarkerListFromText( rawXRef, includeInitialText=True )
            ...
            def handleTextSubsegment( textSegment ):
                ...
                markerList = BibleOrgSysGlobals.USFMMarkers.getMarkerListFromText( textSegment, includeInitialText=True )
...
############################################################################## XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    def toTeX( self, outputFolder=None ):
        ...
        if ignoredMarkers:
            logging.info( "toTeX: Ignored markers were {}".format( ignoredMarkers ) )
            if BibleOrgSysGlobals.verbosityLevel > 2:
    ...
    def doAllExports( self, givenOutputFolderName=None, wantPhotoBible=None, wantODFs=None, wantPDFs=None ):
        ...
        if BibleOrgSysGlobals.debugFlag: # no try/except calls so it halts on errors rather than continuing
            ...
            try: VPLExportResult = self.toVPL( VPLOutputFolder )
                ...
                logging.error( "BibleWriter.doAllExports.toVPL: Oops, failed!" )
            ...
            try: MyBExportResult = self.toMyBible( MyBOutputFolder )
                ...
                logging.error( "BibleWriter.doAllExports.toMyBible: Oops, failed!" )
