...
"""
Module handling OSIS XML Bibles to produce C and Python data tables.
"""
...
class OSISXMLBible( Bible ):
    ...
    def __init__( self, sourceFilepath, givenName=None, encoding='utf-8' ):
        ...
        #self.bookData, self.USFMBooks = OrderedDict(), OrderedDict()
    ...
    def x__init__( self, XMLFilepath ):
        ...
        def processFile( filepath ):
            ...
            thisBookData, thisUSFMData = obc.loadValidateExtract( filepath ) # Load the XML
            for BBB in thisBookData:
                if BBB in self.bookData: logging.warning( _("Have multiple {} OSIS book data").format( BBB ) )
                self.bookData[BBB] = thisBookData[BBB]
        ...
        #self.books = {}
        self.bookData, self.USFMData = OrderedDict(), OrderedDict()
    ...
    def loadFile( self, OSISFilepath ):
        ...
        return self.bookData, self.USFMBooks
    # end of loadFile
            ...
            if element.tag == OSISXMLBible.OSISNameSpace+"revisionDesc":
                ...
                for subelement in element.getchildren():
                    ...
                    if subelement.tag == OSISXMLBible.OSISNameSpace+"title":
                        ...
                        assert( dateType in (None,'Gregorian',) )
                        assert( dateEvent in (None,'eversion',) )
                        ...
                        assert( identifierType in ('OSIS','URL') )
                        ...
                        assert( copyrightType in (None,'x-copyright','x-license','x-license-url',) )
    ...
    def validateFrontMatter( self, frontMatter ):
        ...
        for element in frontMatter:
            if element.tag == OSISXMLBible.OSISNameSpace+"titlePage":
                ...
                for attrib,value in element.items():
                    if attrib=="type":
                        assert( value == 'front' ) # We've already processed this in the calling routine
    ...
    def validateAndExtractMainDiv( self, div ):
                    ...
                    if cmBBB and cmBBB != BBB: # We've started on a new book
                        if BBB and ( len(bookResults)>20 or len(USFMResults)>20 ): # Save the previous book
                            ...
                            if lastUSFMResult[0]!='s':
                                ...
                                lastUSFMResult = None
                            if bookResults: self.bookData[BBB] = bookResults
        ...
        def validateVerseElement( element, verseMilestone, chapterMilestone, locationDescription ):
            ...
            """
            nonlocal haveEIDs
            #print( "validateVerseElement at {} with '{}' and '{}'".format( locationDescription, chapterMilestone, verseMilestone ) )
            ...
            if len(element)==0 and ( sID or eID ): # it's a milestone (no sub-elements)
                ...
                #print( "validateVerseElement bits", bits )
                assert( len(bits)==3 and bits[1].isdigit() and bits[2].isdigit() )
            ...
            if noteText and not noteText.isspace(): # In some OSIS files, this is the anchor reference (in others, that's put in the tail of an enclosed reference subelement)
                ...
                if noteType == 'crossReference': # This could be something like '1:6:' or '1:8: a'
                    ...
                    if ':' in noteText and noteText[0].isdigit(): # Let's roughly assume that it starts with a chapter:verse reference
                        ...
                        assert( sourceText and footnoteText )
            ...
            for subelement in element.getchildren():
                if subelement.tag == OSISXMLBible.OSISNameSpace+"reference": # cross-references
                    ...
                    if noteType=='crossReference' and referenceType=='source':
                        #assert( not noteText and not referenceTail )
                        assert( not noteText or noteText.isspace() )
                        ...
                        if USFMResults and USFMResults[-1][0]=='xt': # Combine multiple cross-references into one xt field
                            USFMResults.append( ('xt',USFMResults.pop()[1]+referenceText+referenceTail,) )
                        else: USFMResults.append( ('xt',referenceText+referenceTail,) )
                        ...
                        assert( referenceText and not noteText.strip() )
                    ...
                    for sub2element in subelement.getchildren(): # Can have nested references in some (horrible) OSIS files)
                        if sub2element.tag == OSISXMLBible.OSISNameSpace+"reference": # cross-references
                            ...
                            assert( noteType == 'crossReference' )
                    ...
                    assert( highlightedText )
                    highlightType = None
                    for attrib,value in subelement.items():
                        if attrib=="type":
                            highlightType = value
                            assert( highlightType == 'italic' )
        ...
        def validateParagraph( element, locationDescription, verseMilestone ):
            ...
            for subelement in element.getchildren():
                if subelement.tag == OSISXMLBible.OSISNameSpace+"chapter": # A chapter break within a paragraph (relatively rare)
                    ...
                    if not noteTail.isspace(): # This is the main text of the verse (follows the inserted note)
                ...
                if 0:
                    ...
                    if titleType=='' or titleType == "main": # Some OSIS files don't have a title type attribute :(
                        if titleShort:
                            ...
                            foundH = True
                        if titleText:
                            assert( len(element) == 0 ) # The title is encoded here not in subelements
                    ...
                    for subelement in element.getchildren():
                        if subelement.tag == OSISXMLBible.OSISNameSpace+"title": # subtitles
                            ...
                            assert( not titleText ) # The text should be specified in the subtitles in this case
...
########### Q
                ...
                for subelement in element.getchildren():
                    if subelement.tag == OSISXMLBible.OSISNameSpace+"verse":
                        ...
                        assert( transchangeType in ('added',) )
                        # Now process the subelements
                        for sub2element in subelement.getchildren():
...
########### Chapter
                ...
                if chapterMilestone.startswith('chapterContainer.'): # it must have been a container -- process the subelements
                    ...
                    assert( len(chapterBits) == 2 )
                    assert( chapterBits[1].isdigit() )
                            ...
                            if verseMilestone and verseMilestone.startswith('verseContainer.'): # it must have been a container -- process the subelements
                                ...
                                for sub2element in subelement.getchildren():
                                    if sub2element.tag == OSISXMLBible.OSISNameSpace+"w":
                                        ...
                                        assert( word ) # That should be the actual word
                                        # Process the attributes
                                        lemma = morph = None
                                        for attrib,value in sub2element.items():
                                        ...
                                        assert( seg ) # That should be the actual segment character
                                        # Process the attributes first
                                        for attrib,value in sub2element.items():
                                            if attrib=="type":
                                                segType = value
...
########### Left-overs!
        ...
        if BBB:
            ...
            #if bookResults: self.bookData[BBB] = bookResults
            #if USFMResults: self.USFMBooks[BBB] = USFMResults
            if USFMResults: self.saveBook( BBB, USFMResults )
    # end of validateAndExtractMainDiv
    ...
    def getVerseDataList( self, reference ):
        ...
        if BBB in self.bookData:
            foundChapter, foundVerse, result = False, False, []
            for info in self.bookData[BBB]:
                if len(info)==2:
                    name, value = info
                    if name == 'chapter':
...
def demo():
    ...
    if 1: # Test OSISXMLBible object
        testFilepath = "../morphhb/wlc/Ruth.xml" # Hebrew Ruth
        #testFilepath = "../morphhb/wlc/Dan.xml" # Hebrew Daniel
        #testFilepath = "../morphhb/wlc/" # Hebrew
        #testFilepath = "../../../../../Data/Work/Bibles/Formats/OSIS/Crosswire USFM-to-OSIS (Perl)/Matigsalug.osis.xml" # Entire Bible in one file 4.4MB
        #testFilepath = "../../../../../Data/Work/Bibles/Formats/OSIS/kjvxml from DMSmith/kjv.xml" # Entire Bible in one file 23.7MB
        #testFilepath = "../../../../../Data/Work/Bibles/Formats/OSIS/kjvxml from DMSmith/kjvfull.xml" # Entire Bible in one file 24.2MB
        #testFilepath = "../../../../../Data/Work/Bibles/Formats/OSIS/kjvxml from DMSmith/kjvlite.xml" # Entire Bible in one file 7.7MB
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output/MBTGEN.xml" # Single book
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output/MBTRUT.xml" # Single book
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output/MBTMRK.xml" # Single book
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output/MBTJAS.xml" # Single book
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output/MBT2PE.xml" # Single book
        #testFilepath = "../../MatigsalugOSIS/OSIS-Output" # Entire folder of single books

        # Demonstrate the OSIS Bible class
        if Globals.verbosityLevel > 1: print( "\nDemonstrating the OSIS Bible class..." )
        ...
        if 1: # Test verse lookup
            for reference in ( ('OT','GEN','1','1'), ('OT','GEN','1','3'), ('OT','PSA','3','0'), ('OT','PSA','3','1'), \
                ...
                #print( reference, ob.getVerseDataList( reference ) )
                print( reference, ob.getVerseText( reference ) )
# end of demo
