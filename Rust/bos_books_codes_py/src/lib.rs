...
use ::bos_books_codes::{
    ...
    get_reference_number, is_ot, is_nt, is_dc, is_valid_reference_abbreviation,
};
...
#[pyfunction]
fn is_ot_py(reference_abbreviation: &str) -> PyResult<bool> {
    Ok(is_ot(reference_abbreviation))
}

/// Returns True if the given reference abbreviation is a New Testament book.
#[pyfunction]
fn is_nt_py(reference_abbreviation: &str) -> PyResult<bool> {
    Ok(is_nt(reference_abbreviation))
}

/// Returns True if the given reference abbreviation is a Deuterocanonical book.
#[pyfunction]
fn is_dc_py(reference_abbreviation: &str) -> PyResult<bool> {
    Ok(is_dc(reference_abbreviation))
}
...
fn bos_books_codes_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(is_ot_py, m)?)?;
    m.add_function(wrap_pyfunction!(is_nt_py, m)?)?;
    m.add_function(wrap_pyfunction!(is_dc_py, m)?)?;
    Ok(())
}
