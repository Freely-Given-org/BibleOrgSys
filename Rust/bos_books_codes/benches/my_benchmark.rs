...
use bos_books_codes::{reference_abbrev_to_usfm_abbrev, usfm_abbrev_to_reference_abbrev, english_name_to_reference_abbrev};

pub fn criterion_benchmark1(c: &mut Criterion) {
    c.bench_function("BBB to USFM to BBB", |b| {
        b.iter(|| {
            usfm_abbrev_to_reference_abbrev(
                reference_abbrev_to_usfm_abbrev(black_box("KI1")).unwrap().unwrap(),
            )
        })
    });
}
